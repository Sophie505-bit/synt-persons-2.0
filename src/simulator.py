...
class Simulator:
    ...
    def _build_questions_block(self) -> str:
        """Форматирует ВСЕ вопросы в один текстовый блок."""
        blocks = []
    ...
    def survey_one(self, persona: PersonaProfile, scenario: str = "",
        ...
        """Опрашивает одну персону по ВСЕМ вопросам за ОДИН вызов LLM."""
        if not self.use_llm:
        ...
        if neighbors_summary:
            neighbors_part = f"\n\nМнения похожих людей (для контекста):\n{neighbors_summary}"
...
СТАТИСТИКА реальных ответов (ориентируйся на неё для реалистичности):
{priors_text}
...
{{"q1": ["1.2", "1.5", ...], "q2": ["2.1", ...], "q3": ["3.5", ...], "q4": ["4.1", ...]}}"""
        ...
        try:
            ...
            # проверяем что все вопросы есть
    ...
    def run_survey(
        ...
        for i, p in enumerate(personas):
            neighbors_summary = ""
            # если есть граф и уже есть ответы — формируем summary
            if neighbors_graph and results and i in neighbors_graph:
                neighbors_summary = self._make_neighbors_summary(
                    neighbors_graph[i], results
                )
    ...
    def _make_neighbors_summary(self, neighbor_indices: List[int],
                                existing_results: List[Dict]) -> str:
        """Агрегирует ответы соседей в краткую сводку."""
        ...
        for qid in ["q1", "q2", "q3", "q4"]:
            if qid in counts:
                ...
                lines.append(f"{qid} популярное: {top_str}")
